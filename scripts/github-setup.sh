#!/usr/bin/env bash
# Configure GitHub repository security settings for context-maxxing.
# Requires: gh auth login (repo scope)
set -euo pipefail

REPO="${1:-gabroberge/context-maxxing}"
BRANCH="${2:-master}"
CI_CHECK="Build"
...
gh api --method PUT "repos/${REPO}/topics" \
  --input - <<'EOF'
{
  "names": ["satire", "astro", "tailwindcss", "ai-agents", "developer-tools", "static-site"]
}
EOF
